...
public class DedupUDAFImp extends DedupUDAFExt {

    static final Log LOG = LogFactory.getLog(DedupUDAFImp.class.getName());
    ...
    @Override
    protected GenericUDAFDedupEvaluator createLLEvaluator() {
        return new GenericUDAFLagEvaluator();
    }

    public static class GenericUDAFLagEvaluator extends GenericUDAFDedupEvaluator {
         ...
         */
        protected GenericUDAFLagEvaluator(GenericUDAFDedupEvaluator src) {
            super(src);
        }
    ...
    static class LagBuffer implements LeadLagBuffer {
        ...
        public void addRow(Object currValue, Object defaultValue) {
            ...
            /*if (row > 1) {
                if (values.get(values.size() - 1).hashCode() == currValue.hashCode()) {
                    currValue = null;
                }
            }*/
            if (currValue == null && values.size() > 0) {
                    values.remove(0);
                    }
    ...
    static class GenericUDAFLagEvaluatorStreaming extends GenericUDAFLagEvaluator
        ...
        protected GenericUDAFLagEvaluatorStreaming(GenericUDAFDedupEvaluator src) {
            super(src);
        }
